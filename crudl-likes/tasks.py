...
def coverage(c):
    ...
    """
    c.run("coverage run --source crudl-likes runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/crudl-likes.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ likes")
...
def lint(c):
    ...
    """
    c.run("flake8 crudl-likes tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=crudl_likes.__version__))
    c.run("git push --tags")
