(function ($, undefined) {
    ...
    var $map, $foundLocations, $lat, $lng, $street, $street2, $city, $country, $postalCode, gMap, gMaker;
    // ...this is where all the further JavaScript functions go...
}(django.jQuery));

function getddress4search() {
...
function updateMarker(lat, lng) {
    ...
    gMaker.setPosition(point);
    gMap.panTo(point, 15);
    gMaker.setDraggable(true);
...
function autocompleteAddress(results) {
    ...
    if (results.length) {
        results.forEach(function (result, i) {
            $link.clone().html(result.formatted_address).click(
                function (event) {
...
function updateAddressFields(addressComponents) {
    var streetName, streetName2;
    var typeActions = {
        "locality": function (obj) {
            $city.val(obj.long_name);
        },
        "street_name": function (obj) {
    ...
    if (streetName) {
        var sStreetAddress = streetName;
        if (streetNumber) {
            sStreetAddress += " " + streetNumber;
        }
        $street.val(sStreetAddress);
    }
}
