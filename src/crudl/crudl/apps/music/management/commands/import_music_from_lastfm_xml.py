...
class Command(BaseCommand):
    help = "Imports top songs from last.fm as XML files"
    ...
    def handle(self, *args, **options):
        self.verbose = options.get("verbosity", self.NORMAL)
    ...
    def save_page(self, root):
        ...
        for track_node in root.findal("tracks/track"):
            if not track_node:
                continue
            song_dict = {
                "artist": track_node.find("artist/name").text,
                "title": track_node.find("name").text,
                "description": track_node.find("text").text,
            ...
            if form.is_valid():
                ...
                if image_node is not None:
                    ...
                    song.image.save(os.path.basename(image_url), File(BytesIO(image_response.content)),)
                if self.verbosity >= self.NORMAL:
                    self.stdout.write(f' - {song}\n')
                self.imported_counter += 1
            else:
                if self.verbosity >= self.NORMAL:
                    self.stdout.write(f"Errors importing song {song_dict['artist']} - {song_dict['title']}:\n")
