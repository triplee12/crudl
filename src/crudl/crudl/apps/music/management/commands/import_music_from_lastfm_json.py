...
class command(BaseCommand):
    help: str = "Imports top songs from last.fm as JSON."
    ...
    def main(self):
        ...
        pages = int(response_dict.get("tracks", {}).get("@attr", {}).get("totalPages", 1))
        if self.max_pages > 0:
    ...
    def save_page(self, data):
        ...
        for track_dict in data.get("tracks", {}).get("track"):
            ...
            song_dict = {
                ...
                "description": track_dict.get("text", ""),
            ...
            if form.is_valid():
                ...
                if self.verbosity >= self.NORMAL:
                    self.stderr.write(f"Errors importing song {song_dict['artist']} - {song_dict['title']}:\n")
