...
class Command(BaseCommand):
    help = (
        ...
        "Expects columns: Artist, Title, Description, Url"
    )
    ...
    def main(self):
        ...
        columns = ["artist", "title", "description", "url"]
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if form.is_valid():
                ...
                if self.verbosity >= self.NORMAL:
                    self.stderr.write(f"Errors importing song {row_dict['artist']} - {row_dict['title']}:\n")
                    self.stderr.write(f"{form.errors.as_json()}\n")
                self.skipped_counter += 1
    
    def finalize(self):
        if self.verbosity >= self.NORMAL:
            self.stdout.write("---------------Finalizing---------------\n")
