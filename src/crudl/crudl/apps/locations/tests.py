...
from crudl.apps.likes.models import Like
...
class LiveLocationTest(LiveServerTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.browser = webdriver.Chrome(executable_path=driver_path, options=chrome_options)
        cls.browser.delete_all_cookies()
        image_path = cls.save_test_image("test.png")
    ...
    def test_login_and_like(self):
        ...
        WebDriverWait(self.browser, timeout=10).until(
            lambda x:
            int(self.browser.find_element_by_css_selector(".like-badge").text) != initial_likes
        )
        likes_in_html = int(
            self.browser.find_element_by_css_selector(".like-badge").text
        )
