...
def load_objects(parser, token):
    ...
    limit_count = None
    try:
        (tag_name, manager_method, str_from, app_model, str_limit, limit_count, str_as, var_name) = token.split_contents()
    except ValueError:
        try:
            (tag_name, manager_method, str_from, app_model, str_as, var_name) = token.split_contents()
        except ValueError:
            tag_name = token.contents.split()[0]
            raise template.TemplateSyntaxError(
                f"{tag_name} tag requires the following syntax: "
                f"{{% {tag_name} [<manager>.]<method> from <app_name>.<model_name> [limit <amount>] as <var_name> %}}"
            )
    try:
        app_name, model_name = app_model.split(".")
    except ValueError:
        raise template.templateSyntaxError(
            "load_objects tag requires application name and model name, separated bt a dot"
        )
    ...
    for key, value_list in context["request"].GET.lists():
        if not key in params_to_remove:
            ...
            if key in params_to_change:
                # update value for keys in params_to_change
