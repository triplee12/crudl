...
class SpecialCharacterInclusionValidator:
    ...
    def validate(self, password, user=None):
        ...
        if not has_specials_chars:
            raise ValidationError(self.get_help_text(pronoun="this"), code="password_missing_special_chars")
    
    def get_help_text(self, pronoun="your"):
        return _(f"{pronoun.capitalize()} password must contain at least one of the following special characters: {', '.join(self.special_chars)}")
